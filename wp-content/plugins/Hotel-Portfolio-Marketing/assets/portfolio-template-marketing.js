(function($) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const grid = document.getElementById("hotel-portfolio-container");
    if (!grid) return;

    const lang = window.location.pathname.includes('/de/') ? 'de' : 'en';
    const targetGroup = grid.dataset.targetGroup || 'b2b';

    fetch(hotelPortfolioMarketing.ajaxurl + `?action=hotel_portfolio_marketing_fetch&lang=${lang}&target_group=${targetGroup}`)
...
//*************FILTER**************************************************/               
                ...
                function generateDropdownOptions(resultHotelArr) {
                  ...
                  const offer = getUniqueSortedValues(['offer_type_01', 'offer_type_02', 'offer_type_03', 'offer_type_04', 'offer_type_05', 'offer_type_06']);
                 // const parentBrand = getUniqueSortedValues(['parent_brand']);
                  ...
                  populateDropdown("offer-options", offer);
                ...
                $("#country-header, #city-header, #brand-header, #offer-header, #parent-brand-header")
                ...
                setupDropdown("offer-header", "offer-options");
...
//************RESET FUNCTION******************************/
                $("#btn-reset").click(function () {
                  ...
                  // Felder zurücksetzen
                  const filters = [
                      { id: "city", placeholder: window.hotelFilterTranslations.city },
                      { id: "country", placeholder: window.hotelFilterTranslations.country },
                      { id: "brand", placeholder: window.hotelFilterTranslations.brand },
                      { id: "offer", placeholder: window.hotelFilterTranslations.offer },
...
//**********SEARCH FUNCTION******************************/
                  function handleEvent() {
                    ...
                    let offer = $("#offer-header").val().trim();
...
//*************MESSAGE CONTAINER/ERROR MESSAGE ***********/
                function message(resultLength) {
                  ...
                  if (resultLength === 0) {
                    ...
                    messageContainer.html(`
                      <div class="message-txt red">${window.hotelFilterTranslations.noResult}</div>  
                    `);
                  ...
                  // if hotels are found
                    ...
                    messageContainer.html(`
                       <div class="message-txt green">
                            <h4 id="message-headline">${window.hotelFilterTranslations.yourSelection}: </h4>
                            <div class="message-filter-result">
                              <div class="result-title" id="title-country"><span class="txt-black">${window.hotelFilterTranslations.country}:</span><span class="txt-gray"> ${globalParams.country}</span></div>
                              <div class="result-title" id="title-city"><span class="txt-black">${window.hotelFilterTranslations.city}:</span><span class="txt-gray"> ${globalParams.city}</span></div>
                              ...
                              <div class="result-title" id="title-brand"><span class="txt-black">${window.hotelFilterTranslations.brand}:</span><span class="txt-gray"> ${globalParams.brand}</span></div>
                              <div class="result-title" id="title-offer"><span class="txt-black">${window.hotelFilterTranslations.offer}:</span><span class="txt-gray"> ${globalParams.offer}</span></div>
                            </div>
                            <div><p class="result-message">${hotelFilterTranslations.searchResultet} <span class="txt-black"> ${resultLength} </span>${window.hotelFilterTranslations.hits}.</p></div>
                          </div>          
                      `);
                ...
                if (globalParams.country && globalParams.country !== "" && globalParams.country !== 'Country') {
                  ...
                  if (globalParams.offer && globalParams.offer !== "" && globalParams.offer !== 'Offer') {
                    $("#title-offer").addClass("show");
                  }
                ...
                function removeShowClass(){
                  let messageTitleArray = ['country', 'city', 'brand', 'offer','parent-brand'];
...
//*************RENDER LIST OF CARDS*************************/
            function renderList(resultHotelArr) {
              ...
              const offerParam = urlParams.get('offer') ? urlParams.get('offer').toLowerCase() : '';
              const container = document.getElementById("hotel-portfolio-container");

              for (let hotel of resultHotelArr) {
                let hotelItem = document.createElement("div");
                hotelItem.classList.add("card-wrapper");

                // HTML for the offer-labels
                let offerLabelsHtml = "";
                for (let i = 1; i <= 6; i++) {
                  let type = hotel[`offer_type_0${i}`] || "";
                  offerLabelsHtml += `<div class="offer-label" data-offer-index="${i}">${type}</div>`;
                }
                ...
                // Hide btn-card if no website
                if (!hotel.website) {
                  hotelItem.querySelector(".btn-card").style.display = "none";
                }
                //offer-labels functionality
                let autoOpened = false;
                  hotelItem.querySelectorAll('.offer-label').forEach(function(label) {
                    const txt = label.textContent.trim();
                    if (txt === "") {
                      label.style.display = "none";
                    } else {
                      let offerId = txt.toLowerCase().replace(/\s+/g, '-');
                      label.setAttribute("id", offerId);

                      const index = label.getAttribute('data-offer-index');
                      const headline = hotel[`offer_title_0${index}`] || '';
                      const desc = hotel[`offer_description_0${index}`] || '';
                      handleOfferClick(label, hotelItem, headline, desc);

                      // --- Auto-open: ---
                      const offerType = txt.toLowerCase();
                      if (!autoOpened && (
                        (offerParam === '' && !autoOpened) ||      // kein offer in URL -> erstes öffnen
                        (offerParam !== '' && offerParam === offerType) // offer-Param stimmt mit Typ überein
                      )) {
                        generateOfferContent(label, hotelItem, headline, desc);
                        autoOpened = true;
                      }
                    }
                  });

                  if (container) container.appendChild(hotelItem);
                }}; 

//*************OFFER FUNCTIONALITY*************************/
                  function handleOfferClick(label, hotelItem, headline, desc) {
                    label.addEventListener('click', function () {
                      generateOfferContent(label, hotelItem, headline, desc);
                    });
                  }
//*************GENERATE OFFER CONTENT*************************/
                function generateOfferContent(label, hotelItem, headline, desc) {
                  // Entferne alle offer-label-active in diesem Hotel-Item
                  hotelItem.querySelectorAll('.offer-label.offer-label-active').forEach(l => {
                    l.classList.remove('offer-label-active');
                  });

                  // Füge die Klasse zum aktuellen Label hinzu
                  label.classList.add('offer-label-active');

                  // Entferne evtl. vorhandene Detailbox
                  const existingBox = hotelItem.querySelector('.offer-detail-box');
                  if (existingBox) existingBox.remove();

                  // Nur anzeigen wenn Inhalt vorhanden
                  if (headline || desc) {
                    const detailBox = document.createElement('div');
                    detailBox.className = 'offer-detail-box';
                    detailBox.style.margin = '1em 0';
                    detailBox.innerHTML = `<h4 class="offer-title">${headline}</h4><p class="offer-description">${desc}</p>`;
                    // Nach offer-tags einfügen
                    const offerTags = hotelItem.querySelector('.offer-tags');
                    offerTags.insertAdjacentElement('afterend', detailBox);
                  }
                }
                ...
                function filterListByParams(params) {
                  ...
                  for (let hotel of fetchedHotels) {
                      ...
                      if (params.offer?.toLowerCase() && !(
                          hotel.offer_type_01?.toLowerCase().includes(params.offer.toLowerCase()) ||
                          hotel.offer_type_02?.toLowerCase().includes(params.offer.toLowerCase()) ||
                          hotel.offer_type_03?.toLowerCase().includes(params.offer.toLowerCase()) ||
                          hotel.offer_type_04?.toLowerCase().includes(params.offer.toLowerCase()) ||
                          hotel.offer_type_05?.toLowerCase().includes(params.offer.toLowerCase()) ||
                          hotel.offer_type_06?.toLowerCase().includes(params.offer.toLowerCase())
                      )) {
                          matchesHotel = false;
                      }
...
//************MAPS VIEW UPDATE FUNCTION******************************/  
                  function updateMapViewBtn() {
                    console.log(hotelPortfolioMarketing.siteUrl);
                    ...
                    console.log(hotelPortfolioMarketing.siteUrl, hotelPortfolioMarketing.lang); 
                    // Start with base path
                    let finalUrl = "";
                    let baseUrl = hotelPortfolioMarketing.siteUrl;
                        if (hotelPortfolioMarketing.lang === 'de_DE') {
