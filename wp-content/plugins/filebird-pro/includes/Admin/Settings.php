...
use FileBird\Addons\PostType\Models\Main as PostTypeModel;
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		if ( $this->hook_suffix === $hook_suffix ) {
			...
			foreach ( $_postTypes as $key => $postType ) {
				if ( in_array( $key, PostTypeModel::EXCLUDED_POST_TYPES, true ) ) {
					continue;
				}
				$postTypes[ $key ] = $postType->labels->singular_name;
			}

			$postTypes = apply_filters( 'filebird_post_types', $postTypes );
