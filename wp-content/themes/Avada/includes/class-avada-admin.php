...
class Avada_Admin {
	 ...
	 * @var string
	 */
	private $theme_url = 'https://avada.website/';
	...
	public function __construct() {
		...
		add_action( 'after_switch_theme', [ 'Avada_Importer_Data', 'set_data_transient' ], 5 );
		...
		if ( ( isset( $_GET['page'] ) && 'avada-setup' === sanitize_text_field( wp_unslash( $_GET['page'] ) ) ) || ( fusion_doing_ajax() && ( strpos( wp_get_referer(), '?page=avada-setup' ) || isset( $_GET['awb_setup_nonce'] ) || isset( $_POST['awb_setup_nonce'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
			$this->init_setup_wizard();
		}

		add_filter( 'leadin_impact_code', [ $this, 'get_hubspot_affiliate_code' ] );

		// Avada site data notice.
		$data_status = get_option( 'awb_site_data_status', [] );
		if ( isset( $data_status['status'] ) && 'dismissed' !== $data_status['status'] ) {
			$this->init_awb_site_data();
		}
	}
	...
	/**
	 * Require and instantiate Avada site data class.
	 *
	 * @access public
	 * @since 5.0.0
	 * @return void
	 */
	public function init_awb_site_data() {
		require_once $this->includes_path . '/class-awb-site-data.php';
	}
	...
	public function admin_menu() {
		...
		if ( current_user_can( 'edit_posts' ) ) {
			...
			if ( ! class_exists( 'FusionReduxFrameworkPlugin' ) ) {
				$theme_options_global = $avada_submenu_page_creation_method( 'themes.php', esc_html__( 'Options', 'Avada' ), esc_html__( 'Options', 'Avada' ), 'manage_options', 'themes.php?page=avada_options' );
			}
	...
	public static function get_admin_screens_header( $screen = 'welcome' ) {
		...
		if ( in_array( $screen, [ 'builder-options', 'layout-sections', 'layouts', 'off-canvas', 'icons', 'forms', 'form-entries', 'library', 'plugins' ], true ) ) {
		...
		} elseif ( in_array( $screen, [ 'support', 'performance', 'setup' ], true ) ) {
			// These screens won't show any notices.
								...
								<ul class="avada-db-menu-sub avada-db-menu-sub-maintenance">
									...
									<li class="avada-db-menu-sub-item avada-db-menu-sub-item-maintenance">
										<a class="avada-db-menu-sub-item-link" href="<?php echo esc_url( admin_url( 'themes.php?page=avada_options#heading_maintenance' ) ); ?>">
											...
											<div class="avada-db-menu-sub-item-text">
												<div class="avada-db-menu-sub-item-label"><?php esc_html_e( 'Maintenance Mode', 'Avada' ); ?></div>
												<div class="avada-db-menu-sub-item-desc"><?php esc_html_e( 'Maintenance & Coming Soon mode.', 'Avada' ); ?></div>
											</div>
										</a>
									</li>
									<?php endif; ?>
									...
									<li class="avada-db-menu-sub-item avada-db-menu-sub-item-support">
										<a class="avada-db-menu-sub-item-link<?php echo ( 'performance' === $screen ) ? ' avada-db-active' : ''; ?>" href="<?php echo esc_url( ( 'performance' === $screen ) ? '#' : admin_url( 'admin.php?page=avada-performance' ) ); ?>">
											...
											<div class="avada-db-menu-sub-item-text">
												...
												<div class="avada-db-menu-sub-item-desc"><?php esc_html_e( 'Optimize your website performance.', 'Avada' ); ?></div>
											</div>
										</a>
									</li>
									<?php endif; ?>
				...
				<footer class="avada-db-footer">
					<div class="avada-db-footer-top">
						<nav class="avada-db-footer-menu">
							...
							<ul>
								<li>
									<a href="https://avada.com/documentation/" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Documentation', 'Avada' ); ?></a>
								</li>
								<li>
									<a href="https://www.youtube.com/@AvadaVideos" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Video Tutorials', 'Avada' ); ?></a>
								</li>
								<li>
									<a href="https://my.avada.com/submit-a-ticket/" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Submit A Ticket', 'Avada' ); ?></a>
								</li>
							</ul>
						</nav>
					...
					<div class="avada-db-footer-bottom">
						...
						<nav class="avada-db-footer-menu-bottom">
							<a href="https://avada.com/support-policy/" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Support Policy', 'Avada' ); ?></a>
						</nav>
					</div>
				</footer>
			<?php endif; ?>
	...
	public function admin_scripts() {
		...
		wp_enqueue_style( 'avada-wp-admin-css', get_template_directory_uri() . '/assets/admin/css/admin.css', [], $version );
	...
	private function get_admin_script_l10n_strings() {
		$import_warning_default = __( '<p>Importing prebuilt content will give you layouts, pages, posts, forms, icons, global options, widgets, sidebars, sliders and other settings. This will replicate the live prebuilt site. <strong>Clicking this option will replace your current global options and widgets.</strong> It can also take a minute to complete.</p><h4 style="text-transform:uppercase;">' . esc_html__( 'Recommendations', 'Avada' ) . '</h4><p class="requirement"><i class="req-icon"></i><span class="req-text">Memory Limit of 128 MB and max execution time (php time limit) of 180 seconds.</span></p><h4 style="text-transform:uppercase;">' . esc_html__( 'Requirements', 'Avada' ) . '</h4></p><p class="requirement"><i class="req-icon"></i><span class="req-text">Avada Core must be activated for Avada Sliders, portfolios and FAQs to import.</span></p><p class="requirement"><i class="req-icon"></i><span class="req-text">Avada Builder must be activated for layouts, icons, forms and page content to display as intended.</span></p><p class="requirement"><i class="req-icon"></i><span class="req-text">Any other plugins that are listed as required need to be active in order to install the corresponding third-party content.</span></p>', 'Avada' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
	...
	public function legacy_countdown() {
		...
		if ( ! Avada()->registration->is_registered() && '' !== Avada()->registration->get_token() && isset( $data['legacy_end'] ) && '' !== $data['legacy_end'] && ! Avada()->registration->is_legacy_update() ) {
			...
			$link    = '<a href="https://avada.com/documentation/avada-registration-and-licensing-faq/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Avada Registration Q and A', 'Avada' ) . '</a>';
